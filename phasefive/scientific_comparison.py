...
def perform_comparison():
    ...
    for k in K_VALUES:
        print(f"
--- Scientific Comparison for K={k} ---")
        categories = get_top_k_categories(k)
        X_train, X_test, y_train, y_test = load_plankton_k_categories(categories, img_size=(4, 4))
        ...
        X_test_q = tfq.convert_to_tensor([convert_to_circuit(x) for x in X_test])
        ...
        for t in range(NUM_TRIALS):
            ...
            c_pred = np.argmax(c_model.predict(X_test), axis=1)
    ...
    plt.title('Accuracy Comparison (Swept)')
    plt.xticks(K_VALUES)
    plt.legend()
    ...
    plt.title('F1-Score Comparison (Swept)')
    plt.xticks(K_VALUES)
    plt.legend()
    plt.grid(True, alpha=0.3)
