...
def run_k_experiment(k):
    ...
    # Pre-convert PCA features to circuits
    ...
    for trial in trial_pbar:
        ...
        # (shortened for brevity in tool call, matching exact structure)
        ...
        start = time.time()
        fair_nn.fit(X_train_pca[:q_limit], y_train[:q_limit], epochs=EPOCHS, batch_size=BATCH_SIZE, verbose=0)
        ...
        start = time.time()
        cnn.fit(X_train_raw[:q_limit, ..., np.newaxis], y_train[:q_limit], epochs=EPOCHS, batch_size=BATCH_SIZE, verbose=0)
